...
class PasswordResetToken {
 ...
 @Column(nullable=false,index=false) Long ownerId;
...
public class PasswordResetController {
 ...
 public PasswordResetController(OwnerAccountRepository owners,PasswordResetTokenRepository tokens,JavaMailSender mailSender){this.owners=owners;this.tokens=tokens;this.mailSender=mailSender;}

 @GetMapping("/status") public Map<String,Object> status(){return Map.of("enabled",enabled);}
 ...
 @PostMapping("/request") public ResponseEntity<?> request(@RequestBody Map<String,String> body){
  ...
  if(enabled&&!email.isBlank())owners.findByEmailIgnoreCase(email).ifPresent(this::issueReset);
 ...
 @PostMapping("/confirm") public ResponseEntity<?> confirm(@RequestBody Map<String,String> body){
  if(!enabled)return ResponseEntity.status(503).body(Map.of("error","Password reset is not configured."));
  ...
  if(found.isEmpty()||found.get().expiresAt.isBefore(LocalDateTime.now()))return ResponseEntity.status(400).body(Map.of("error","This reset link is invalid or expired."));
  PasswordResetToken t=found.get();OwnerAccount owner=owners.findById(t.ownerId).orElse(null);
  if(owner==null)return ResponseEntity.status(400).body(Map.of("error","This reset link is invalid or expired."));
  owner.setPasswordHash(encoder.encode(password));owners.save(owner);t.used=true;tokens.save(t);
 ...
 private void issueReset(OwnerAccount owner){
  ...
  try{mailSender.send(msg);}catch(Exception ex){t.used=true;tokens.save(t);}
 }
