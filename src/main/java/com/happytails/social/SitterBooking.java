...
interface SitterBookingRepository extends JpaRepository<SitterBooking,Long>{
  ...
  List<SitterBooking> findBySitterId(Long sitterId);
}
...
class SitterBookingController {
  ...
  private boolean live(SitterBooking b){return !"CANCELLED".equalsIgnoreCase(b.getStatus())&&!"DECLINED".equalsIgnoreCase(b.getStatus());}
  private boolean overlaps(SitterBooking b,LocalDateTime start,LocalDateTime end){return live(b)&&b.getStartAt()!=null&&b.getEndAt()!=null&&start.isBefore(b.getEndAt())&&end.isAfter(b.getStartAt());}
  ...
  @PostMapping ResponseEntity<?> create(@RequestBody SitterBooking b,HttpSession s){Long pet=active(s);if(pet==null)return ResponseEntity.status(401).body(Map.of("error","login required"));if(b.getSitterId()==null||!sitters.existsById(b.getSitterId()))return ResponseEntity.badRequest().body(Map.of("error","Sitter not found."));PetSitter sitter=sitters.findById(b.getSitterId()).orElseThrow();if(!sitter.isAvailable())return ResponseEntity.status(409).body(Map.of("error","This sitter is currently unavailable."));if(b.getStartAt()==null||b.getEndAt()==null||!b.getEndAt().isAfter(b.getStartAt()))return ResponseEntity.badRequest().body(Map.of("error","Choose a valid start and end time."));if(b.getStartAt().isBefore(LocalDateTime.now()))return ResponseEntity.badRequest().body(Map.of("error","Booking must be in the future."));if(bookings.findBySitterId(b.getSitterId()).stream().anyMatch(x->overlaps(x,b.getStartAt(),b.getEndAt())))return ResponseEntity.status(409).body(Map.of("error","This sitter already has a request or booking during that time. Choose another time."));b.setPetProfileId(pet);b.setStatus("PENDING");return ResponseEntity.status(201).body(bookings.save(b));}

  @PatchMapping("/{id}/cancel") ResponseEntity<?> cancel(@PathVariable Long id,HttpSession s){Long pet=active(s);if(pet==null)return ResponseEntity.status(401).build();return bookings.findById(id).<ResponseEntity<?>>map(b->{if(!Objects.equals(b.getPetProfileId(),pet))return ResponseEntity.status(403).body(Map.of("error","Not your booking."));if("CANCELLED".equalsIgnoreCase(b.getStatus()))return ResponseEntity.ok(b);if("DECLINED".equalsIgnoreCase(b.getStatus()))return ResponseEntity.status(409).body(Map.of("error","A declined booking cannot be cancelled."));if(b.getEndAt()!=null&&b.getEndAt().isBefore(LocalDateTime.now()))return ResponseEntity.status(409).body(Map.of("error","Past bookings cannot be changed."));b.setStatus("CANCELLED");return ResponseEntity.ok(bookings.save(b));}).orElseGet(()->ResponseEntity.notFound().build());}

  @PatchMapping("/{id}/confirm-demo") ResponseEntity<?> confirmDemo(@PathVariable Long id,HttpSession s){Long pet=active(s);if(pet==null)return ResponseEntity.status(401).build();return bookings.findById(id).<ResponseEntity<?>>map(b->{if(!Objects.equals(b.getPetProfileId(),pet))return ResponseEntity.status(403).body(Map.of("error","Not your booking."));if(!"PENDING".equalsIgnoreCase(b.getStatus()))return ResponseEntity.status(409).body(Map.of("error","Only pending requests can be confirmed."));if(b.getStartAt()!=null&&b.getStartAt().isBefore(LocalDateTime.now()))return ResponseEntity.status(409).body(Map.of("error","Past requests cannot be confirmed."));if(!sitters.existsById(b.getSitterId())||!sitters.findById(b.getSitterId()).orElseThrow().isAvailable())return ResponseEntity.status(409).body(Map.of("error","This sitter is no longer available."));b.setStatus("CONFIRMED");return ResponseEntity.ok(bookings.save(b));}).orElseGet(()->ResponseEntity.notFound().build());}
}
