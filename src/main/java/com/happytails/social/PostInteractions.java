...
class PostInteractionController{
  ...
  private boolean areFriends(Long a,Long b){return a!=null&&b!=null&&!blocked(a,b)&&(friends.existsByFromPetIdAndToPetIdAndStatus(a,b,"ACCEPTED")||friends.existsByFromPetIdAndToPetIdAndStatus(b,a,"ACCEPTED"));}
  private boolean canSee(Long viewer,SocialPost p){if(Objects.equals(viewer,p.getPetProfileId()))return true;if(viewer!=null&&blocked(viewer,p.getPetProfileId()))return false;Optional<PetProfile> owner=profiles.findById(p.getPetProfileId());return owner.isPresent()&&(!owner.get().isPrivateAccount()||areFriends(viewer,p.getPetProfileId()));}
