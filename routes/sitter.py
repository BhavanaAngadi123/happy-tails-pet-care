...
def register():
    if request.method == "POST":
        ...
        if Sitter.query.filter_by(email=email).first():
            flash("Email already registered!", "warning")
            return redirect(url_for("sitter.register"))
        sitter = Sitter(name=name, email=email, phone=phone, service_types=[], verification_status="pending", profile_image="default-avatar.png")
        sitter.set_password(password)
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            sitter.profile_image = save_file(file, UPLOAD_FOLDER, "sitter-profiles")
        id_file = request.files.get("id_document")
        if id_file and id_file.filename:
            sitter.id_document = save_file(id_file, ID_FOLDER, "sitter-ids")
        selfie_file = request.files.get("selfie_with_id")
        if selfie_file and selfie_file.filename:
            sitter.selfie_with_id = save_file(selfie_file, SELFIE_FOLDER, "sitter-selfies")
        db.session.commit()
...
def add_pricing():
    ...
    rule = PricingRule(sitter_id=sitter.id, service_name=service_name, pet_size=pet_size, duration=int(duration), special_needs=special_needs, price=float(price))
    db.session.add(rule)
    db.session.commit()
...
def set_availability():
    ...
    if request.method == "POST":
        ...
        slot = Availability(sitter_id=sitter_id, date=date, start_time=start_time, end_time=end_time, notes=notes)
        db.session.add(slot)
        db.session.commit()
